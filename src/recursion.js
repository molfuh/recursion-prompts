...
var sum = function(array) {
    if (array.length === 0) {
        ...
        return array[0] + Number(sum(array.slice(1)));
    }
};
...
var reverse = function(string) { //traf
    if (string === '') {
        return '';
    } else {
...
var palindrome = function(string) {
    if (string.length === 0 || string.length === 1) {
        return true;
    } else if (string.split(' ').join('').split('').pop().toLowerCase() !== string.split(' ').join('').split('').shift().toLowerCase()) {
        return false;
    } else {
...
var createArray = function(str) {
    ...
    var newArr = [];
    newArr = newArr.concat(str[0], createArray(str.slice(1)));
    return newArr;
};
...
var reverseArr = function(array) {
    ...
    var newArr = [];
    return newArr = newArr.concat(reverseArr(array.slice(1)), array[0]);
};
...
var buildList = function(value, length) {
    ...
    var newArr = [value];
    return newArr = newArr.concat(buildList(value, length = length - 1));
};
...
var fizzBuzz = function(n) {
    ...
    var newArr = [];
    return newArr.concat(fizzBuzz(n - 1), current);
};
...
var rMap = function(array, callback) {
    ...
    var newArr = [];
    return newArr.concat(callback(array[0]), rMap(array.slice(1), callback));
};
...
if (objVals.indexOf(value) > -1) {
    count++;
}
// base case (core functionality)
  // if my current input has this key,
    // increment total
// loop through input
...
//     count++;
//   }
  // recursively call on inner objects
  if (typeof(obj[current]) === 'object') {
      count += countValuesInObj(obj[current], value);
  }
}
// return total
return count;
...
var fibonacci = function(n) {
    ...
    if (n < 1) {
        ...
        var currentArr = fibonacci(n - 1);
        return result.concat(currentArr, currentArr[currentArr.length - 2] + currentArr[currentArr.length - 1]);
    }
};
...
var capitalizeWords = function(array) {
    ...
    var capital = [];
    return capital.concat(array[0].toUpperCase(), capitalizeWords(array.slice(1)));
  };
...
var capitalizeFirst = function(array) {
    ...
    var capital = [];
    //concat first letter of current index
    return capital.concat(array[0][0].toUpperCase() + array[0].slice(1), capitalizeFirst(array.slice(1)));
  };
...
var nestedEvenSum = function(obj) {
    ...
    for (var current in obj) {
    //if current value in object is even,
      if (obj[current] % 2 === 0) {
        //add to total
        total = obj[current];
      } else if (typeof(obj[current]) === 'object') {
    //if current key has an object for its value
        total += nestedEvenSum(obj[current]);
